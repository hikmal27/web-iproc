<template>
    <q-page>
        <div class="tw-flex tw-items-center tw-justify-between tw-px-4 tw-py-4 tw-border-b lg:tw-py-6 dark:tw-border-primary-darker">
            <h1 class="tw-text-2xl tw-font-semibold">Detail Permission</h1>
        </div>
...
export default defineComponent({
    ...
    mounted() {
        this.getDetailPermissions()
    },
    methods: {
        getDetailPermissions() {
            let vm = this

            vm.$api.get('/permissions/' + vm.$route.params.id)
                .then(ress => {
        ...
        editPermission() {
            ...
            vm.$api.put('/permissions/' + vm.$route.params.id, data)
                .then(ress => {
                    console.log(ress)
                    if(ress.status == 200) {
                        vm.$q.notify({
                        ...
                        vm.$q.notify({
                            message: 'Permission cant be updated!',
                            color: 'negative'
                        })
                    }
                })
